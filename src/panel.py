...
class FilePathPanel(Panel):
    def __init__(self, parent, path_string):
        ...
        ctk.CTkButton(self, text = "Open File Explorer", command = self.open_file_dialog).pack(pady = 5)
...
class SaveButton(ctk.CTkButton):
    def __init__(self, parent, export_iamge,name_string, file_string, path_string):
        super().__init__(parent, text = "Export", command = self.save)

        self.export_image = export_iamge
        self.name_string = name_string
        self.file_string = file_string
        self.path_string = path_string

        self.pack(expand = True, side = "bottom", padx = 5, pady = 5)
